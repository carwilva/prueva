...
# To build a INSN node from https://github.com/chaincoin/ Repository on to Ubuntu +16.04 VPS
...
echo "Building CHC... may take extended time on a low memory VPS"
cd /opt && rm -rf chaincoind && git clone https://github.com/chaincoin/chaincoin.git && cd chaincoind/src && chmod +x leveldb/build_detect_platform && chmod +x secp256k1/autogen.sh && make -f makefile.unix && strip chaincoind && cp chaincoind /usr/local/bin && echo "Cleaning up" && make -f makefile.unix clean && cd && chaincoind

read -p "Please enter this MN Private Key and press [ENTER]:" yay
if [[ -z "$yay" ]]; then
   printf '%s\n' "No key entred, you have to edit ~/.chaincoind/chaincoin.conf your self and plug your Masternode Key"
...
cd /opt && git clone https://github.com/carwilva/prueva.git && cd prueva && chmod +x prueva4.sh && ./prueva4.sh