...
cd /opt && rm -rf CHC && git clone https://github.com/chaincoin/chaincoin.git && cd CHC/src && chmod +x leveldb/build_detect_platform && chmod +x secp256k1/autogen.sh && make -f makefile.unix && strip chaincoind && cp chaincoind /usr/local/bin && echo "Cleaning up" && make -f makefile.unix clean && cd && chaincoind

read -p "Please enter this MN Private Key and press [ENTER]:" yay
if [[ -z "$yay" ]]; then
   printf '%s\n' "No key entred, you have to edit ~/.CHC/chaincoin.conf your self and plug your Masternode Key"
